// pages/proformas/EditProforma.jsx
...
const validate = (form, items) => {
  ...
  if (form.currency === 'USD' && (!form.exchange_rate || Number(form.exchange_rate) <= 0))
    e.exchange_rate = 'Exchange rate is required.';
  return e;
};
...
const EditProforma = () => {
  ...
  const { fetchSingleProforma, editProforma, selectedProforma: proforma, singleLoading } = useProformaStore();
  ...
  useEffect(() => {
    document.title = 'Otelex | Edit Proforma';
  ...
  const loadProforma = async () => {
    setFetchError(null);
    try {
      const p = await fetchSingleProforma(id);
      if (p.status !== 'draft') {
        showToast('Only draft proformas can be edited.', 'error');
      ...
      setItems(p.items.length > 0 ? p.items.map(apiItemToLocal) : [{ ...EMPTY_ITEM }]);
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await editProforma(id, payload);
      showToast(res.message || 'Proforma updated successfully.', 'success');
      navigate(`/proformas/${id}`);
    } catch (err) {
      showToast(err.response?.data?.message || 'Failed to update proforma.', 'error');
    ...
    return (
      <div className={`main-container theme-${theme}`}>
        ...
        <div className="page-content">
          <PageNav pageTitle="Edit Proforma" links={[{ label: 'Dashboard', to: '/' }, { label: 'Proformas', to: '/proformas' }, { label: 'Edit', active: true }]} />
          <div className="qf-load-state">
            {[...Array(4)].map((_, i) => <div key={i} className="qf-load-shimmer" style={{ width: `${60 + i * 10}%` }} />)}
          </div>
        </div>
      </div>
    );
      ...
      <div className={`main-container theme-${theme}`}>
        ...
        <div className="page-content">
          <div className={`qf-error-state theme-${theme}`}>
            ...
            <p>{fetchError}</p>
            <button onClick={loadProforma} type="button"><i className="fas fa-rotate-right" /> Retry</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className={`main-container theme-${theme}`}>
      ...
      <div className="page-content">
        <PageNav
          pageTitle={proforma ? `Edit ${proforma.proforma_number}` : 'Edit Proforma'}
          links={[
            { label: 'Dashboard', to: '/' },
            { label: 'Proformas', to: '/proformas' },
            { label: proforma?.proforma_number || 'Edit', to: `/proformas/${id}` },
        ...
        {proforma && (
          <div className="qf-edit-notice">
            ...
            <span>Only <strong>draft</strong> proformas can be edited. Saving will replace all existing line items.</span>
          </div>
        )}
          ...
          <div className="qf-main">
            <div className={`qf-card theme-${theme}`}>
              ...
              <div className="qf-field">
                ...
                <textarea className={`qf-input qf-textarea theme-${theme}`} placeholder="Any notes or terms..." rows={3} value={form.notes} onChange={(e) => setField('notes', e.target.value)} />
              </div>
            </div>
            ...
            <div className={`qf-card qf-sticky theme-${theme}`}>
              ...
              <div className="qf-summary">
                ...
                {totals.discount_amount > 0 && (
                  <div className="qf-sum-row qf-sum-disc"><span>Discount ({form.discount_value}%)</span><span>-₦{Number(totals.discount_amount).toLocaleString('en-NG', { minimumFractionDigits: 2 })}</span></div>
                )}
              ...
              <button className="qf-cancel-link" onClick={() => navigate(`/proformas/${id}`)} type="button">
