// pages/proformas/RejectProformaModal.jsx
import React, { useState, useEffect, useRef } from 'react';
import { createPortal } from 'react-dom';
import useThemeStore from '../../stores/useThemeStore';

const RejectProformaModal = ({ open, proformaNumber, onClose, onConfirm, loading }) => {
    ...
    <div className={`rm-backdrop ${active ? 'rm-visible' : ''}`} onClick={() => !loading && onClose()}>
      ...
      >
        <div className="rm-header">
          <div className="rm-header-icon"><i className="fas fa-circle-xmark" /></div>
          <div>
            <h4 className="rm-title">Reject Proforma</h4>
            <p className="rm-sub">{proformaNumber}</p>
          </div>
        ...
        <div className="rm-body">
          ...
          <textarea
            ...
            placeholder="e.g. Pricing needs revision, items changed..."
            value={reason}
          ...
          <p className="rm-note">
            ...
            The reason is logged in the activity trail. The client is not notified automatically.
          </p>
        </div>
        <div className="rm-actions">
          ...
          <button className="rm-btn rm-confirm" onClick={() => onConfirm(reason)} disabled={loading} type="button">
            {loading ? <><span className="rm-spinner" /> Rejecting...</> : <><i className="fas fa-circle-xmark" /> Reject Proforma</>}
          </button>
        </div>
      </div>
    </div>,
...
export default RejectProformaModal;
