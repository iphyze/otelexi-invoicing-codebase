...
import PreviewQuotationPDF from '../../components/pdf/PreviewQuotationPDF';
...
const PreviewQuotation = () => {
  ...
  const { fetchSingleQuotation, selectedQuotation: Quotation, singleLoading } = useQuotationStore();
  ...
  useEffect(() => {
    ...
    fetchSingleQuotation(id).catch((err) => {
      setFetchError(err.response?.data?.message || 'Failed to load quotation preview.');
    });
  }, [id]);
  ...
  return (
    <div className={`main-container theme-${theme}`}>
      ...
      <div className="page-content">
        <PageNav
          pageTitle={`Quotation Preview - ${inv?.quotation_number ||inv?.proforma_number}`}
          links={[
            { label: 'Dashboard', to: '/' }, 
            ...
            <div className="preview-container">
              ...
              <div className={`sinv-hero theme-${theme}`} style={{justifyContent: 'flex-end'}}>
                ...
                <div className="sinv-hero-actions">
                  ...
                  <PDFDownloadButton type="quotation" doc={inv} label="Download PDF" />

                </div>
              
              </div>

              <PreviewQuotationPDF doc={inv}/>
